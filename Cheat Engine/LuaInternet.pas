...
begin
  if lua_gettop(l)>0 then
    luaclass_newClass(L, TWinInternet.create({$ifdef altname}'Arc Debugger'{$else}cename{$endif}+' : luascript-'+Lua_ToString(L,1)))
  else
    luaclass_newClass(L, TWinInternet.create({$ifdef altname}'Arc Debugger'{$else}cename{$endif}+' : luascript'));
